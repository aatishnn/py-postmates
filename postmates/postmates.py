...
class PostmatesAPI(object):
    ...
    def _base_url(self):
        return '%s/%s/customers/%s' % (
            self.BASE_URL, self.version, self.customer_id)
...
class Delivery(object):
    ...
    def __repr__(self):
        ...
        if self.status != Delivery.STATUS_UNSUBMITTED:
            ...
            s.append('Dropoff ETA: %s' % _to_local_tz(self.dropoff_eta))
            s.append('Dropoff Deadline: %s'
                     % _to_local_tz(self.dropoff_deadline))
            s.append('Fee: $%.2f %s' % (self.fee / 100.0,
                                        self.currency))
...
class Location(object):
    ...
    def __init__(
        self, name, address, phone_number, business_name=None, notes=None
    ):
        self.name = name
